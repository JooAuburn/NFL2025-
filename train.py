...
parser.add_argument('--n_stgcnn', type=int, default=7,help='Number of ST-GCNN layers')
...
parser.add_argument('--pred_seq_len', type=int, default=26)
...
# Step 3: Initialize the TrajectoryDatasetWithText class
dset_train = TrajectoryDatasetWithText(
    data_dir=data_set + 'train/train.txt',  # Path to trajectory data
    text_file=data_set + 'train/train_text.txt',  # Path to text data
...
dset_val = TrajectoryDatasetWithText(
        data_dir=data_set + 'val/valid.txt',  # Path to trajectory data
        text_file=data_set + 'val/valid_text.txt',  # Path to text data
        obs_len=obs_seq_len,                    # Observation length
        pred_len=pred_seq_len,                  # Prediction length
        skip=36,                                # Skip frames
        norm_lap_matr=True,                     # Normalize Laplacian matrix
...
# Optional: Iterate through the DataLoader to check the output
for batch in loader_train:
    obs_traj, pred_traj, obs_traj_rel, pred_traj_rel, non_linear_ped, loss_mask, v_obs, A_obs, v_pred, A_pred, text_features = batch
    print("Observation Trajectory Shape:", obs_traj.shape)
